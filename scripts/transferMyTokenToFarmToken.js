...
module.exports = async function (callback) {
  ...
  // through transferFrom. This is 0 by default
  ...
  //   In order to allow the smart contract to transfer to MyToken (ERC-20) on the accounts[0] behalf, we must
  //   explicitly allow it. We allow FarmToken to tranfer x amount of MyToken on our behalf
  ...
  console.log(
    `Balance MyToken Before TokenFarm: ${web3.utils.fromWei(
      balanceMyTokensBeforeFarmToken.toString()
    )}`
  );
  ...
  console.log(
    `Balance MyToken After accounts[0]: ${web3.utils.fromWei(
      balanceMyTokenAfterAccounts0.toString()
    )} `
  );
  console.log(
    `Balance MyToken After TokenFarm: ${web3.utils.fromWei(
  ...
  console.log(
    `Balance FarmToken After accounts[0]: ${web3.utils.fromWei(
      balanceFarmTokenAfterAccounts0.toString()
    )}`
  );

  console.log(
    `Balance Farm Token After TokenFarm: ${web3.utils.fromWei(
